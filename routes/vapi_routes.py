...
# --- ENV / clients for intent classification ---
...
emb_client = OpenAI(api_key=Config.OPENAI_API_KEY)
or_client = OpenAI(api_key=OPENROUTER_API_KEY, base_url=OPENROUTER_BASE_URL) if OPENROUTER_API_KEY else None
...
def _json_string(obj) -> str:
    return json.dumps(obj, ensure_ascii=False)

def translate_to_english(text: str) -> tuple[str, int]:
    if not text: return "", 0
    if not or_client:
        logger.warning("OpenRouter client not available for translation")
        return text, 0
    t0 = time.time()
    try:
        resp = or_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a translator. Translate user text to neutral English. Return only the translation."},
...
def embed_english(text: str) -> tuple[list[float], int, str]:
    ...
    try:
        ...
        return [], 0, "text-embedding-3-small"

def match_topk(client_id: str, vec: list[float], k: int) -> list[dict]:
    try:
        vapi_service = VAPIWebhookService()
        r = vapi_service.supabase.rpc("match_intents", {"client_row_id": client_id, "query_embedding": vec, "match_count": k}).execute()
        if r.error: raise RuntimeError(r.error.message)
        return r.data or []
    except Exception as e:
        logger.error(f"Vector match failed: {e}")
        return []
...
def load_intents(intent_ids: list[str]) -> list[dict]:
    ...
    try:
        vapi_service = VAPIWebhookService()
        r = vapi_service.supabase.table("intent").select(
        ...
        logger.error(f"Load intents failed: {e}")
        return []

def load_category(category_id: str | None) -> dict | None:
    if not category_id: return None
    try:
        vapi_service = VAPIWebhookService()
        r = vapi_service.supabase.table("intent_category").select(
        ...
        logger.error(f"Load category failed: {e}")
        return None

def get_curated_clarifier(a: str, b: str) -> str | None:
    try:
        vapi_service = VAPIWebhookService()
        cond = f"and(intent_id_a.eq.{a},intent_id_b.eq.{b}),and(intent_id_a.eq.{b},intent_id_b.eq.{a})"
        r = vapi_service.supabase.table("intent_clarifier").select("question,intent_id_a,intent_id_b").or_(cond).maybe_single().execute()
        return None if getattr(r, "error", None) else (r.data or {}).get("question")
    except Exception as e:
        logger.error(f"Get clarifier failed: {e}")
        return None

def classify_with_openrouter(utter_en: str, candidates: list[dict], target_language: str | None) -> dict:
    if not or_client:
        logger.warning("OpenRouter client not available for classification")
        return {
            "best_intent_id": candidates[0]["id"] if candidates else None,
            "confidence": 0.5,
...
def get_client_config(client_id: str) -> dict:
    try:
        vapi_service = VAPIWebhookService()
        r = vapi_service.supabase.table("client_call_router_config").select("*").eq("client_id", client_id).maybe_single().execute()
        if getattr(r, "error", None):
            return {"log_utterance": True, "log_translation": True, "redact_pii": True, "retention_days": 90}
        row = r.data or {}
        return {
            "log_utterance": row.get("log_utterance", True),
            "log_translation": row.get("log_translation", True),
            "redact_pii": row.get("redact_pii", True),
            "retention_days": row.get("retention_days", 90)
        }
    except Exception as e:
        logger.error(f"Get client config failed: {e}")
        return {"log_utterance": True, "log_translation": True, "redact_pii": True, "retention_days": 90}

def resolve_call_id_from_webhook(vapi_webhook_event_id: str) -> str:
    """Try to find Twilio call_sid via vapi_webhook_event_id; fallback to the webhook id itself."""
    try:
        _ = uuidlib.UUID(vapi_webhook_event_id)  # validate UUID format
    except Exception:
        return vapi_webhook_event_id  # leave as-is if not UUID-ish
    try:
        vapi_service = VAPIWebhookService()
        r = vapi_service.supabase.table("twilio_call").select("call_sid").eq("vapi_webhook_event_id", vapi_webhook_event_id).maybe_single().execute()
        if getattr(r, "error", None) or not r.data:
            return f"VAPI:{vapi_webhook_event_id}"
        return r.data.get("call_sid") or f"VAPI:{vapi_webhook_event_id}"
    except Exception as e:
        logger.error(f"Resolve call ID failed: {e}")
        return f"VAPI:{vapi_webhook_event_id}"

def effective_policy(intent_row: dict, category_row: dict | None) -> dict:
...
@vapi_bp.route('/classify-intent', methods=['POST'])
def classify_intent_route():
    ...
    try:
        ...
        if detected_lang != "en":
            utter_en, translate_ms = translate_to_english(user_text)
        else:
            utter_en, translate_ms = user_text, 0
        ...
        # --- logging (rich, but PII-redacted by default) ---
        cfg = get_client_config(client_id)
        call_id = resolve_call_id_from_webhook(vapi_event_id)
        payload = {
            "client_id": client_id,
            "call_id": call_id,
        ...
        if cfg.get("log_utterance", True):
            payload["utterance"] = _redact_pii(user_text) if cfg.get("redact_pii", True) else user_text
            payload["detected_lang"] = detected_lang
        if cfg.get("log_translation", True):
            payload["utterance_en"] = _redact_pii(utter_en) if cfg.get("redact_pii", True) else utter_en
        try:
            vapi_service = VAPIWebhookService()
            vapi_service.supabase.table("call_reason_log").insert(payload).execute()
        except Exception as e:
            logger.warning(f"Failed to log to call_reason_log: {e}")  # don't break the call if logging fails

        # --- shape response (result must be a string) ---
        result_obj = {
